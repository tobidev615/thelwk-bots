...
class PMNewsPipeline(object):
    ...
    def process_item(self, item, spider):
        ...
        page = item.get('page')
        commando="INSERT INTO pm_store_import(title, sql_date, image, thumb, body, link, category, source, page) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s)"
        self.curr.execute('SAVEPOINT last_save')
        try: 
            self.curr.execute(commando,(title, date_sql, pictures, thumb, story, linker, category, source, page))
        except:
...
class AllNewsPipeline(object):
    ...
    def open_spider(self, spider):
        self.conn = pymysql.connect(host=self.host ,user=self.user,password=self.passwd,db='thelwk-databox')
        self.curr= self.conn.cursor()
        self.conn.commit()
    ...
    def process_item(self, item, spider):
        ...
        source = item.get('source')
        page = item.get('page')
        commando="INSERT INTO allnewsng(title, sql_date, brief, body, category, source, image,  link,  page) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s)"
        self.curr.execute('SAVEPOINT last_save')
        try: 
            self.curr.execute(commando,(title, date_sql, brief, body, category, source, image, link, page))
        except:
            self.curr.execute('ROLLBACK TO SAVEPOINT last_save')        
        self.conn.commit()
        return item
